...
func genRegistration(cfg Config, resources []Resource) ([]byte, error) {
	...
	for _, r := range resources {
		if r.Assignment {
			continue // assignment resources emit no data source
		}
...
func genResource(cfg Config, r Resource) ([]byte, error) {
	if r.Assignment {
		return genAssignmentResource(cfg, r)
	}
	var b bytes.Buffer
	recv := r.recv()
	model := r.model()
...
// assignmentUserRead is the bindings-service method that reads a user's effective
// assignment for the policy type (default "EffectiveUserPolicy").
func (r Resource) assignmentUserRead() string {
	if r.AssignmentUserRead != "" {
		return r.AssignmentUserRead
	}
	return "EffectiveUserPolicy"
}

// grantUserField is the grant op's params field that carries the target user
// (default "Identity").
func (r Resource) grantUserField() string {
	if r.Create.IdentityField != "" {
		return r.Create.IdentityField
	}
	return "Identity"
}

// genAssignmentResource emits a per-user policy-assignment resource (see
// Resource.Assignment). It manages a Grant-Cs<Type>Policy user grant as a
// two-field resource (user + policy_name) with no object New/Get/Set/Remove
// lifecycle: Create/Update grant the named instance and poll until effective,
// Delete grants "" (unassign), and Read derives the effective assignment. It is
// self-contained (mirrors genCreate/genRead/... but for the grant transport).
func genAssignmentResource(cfg Config, r Resource) ([]byte, error) {
	var b bytes.Buffer
	recv := r.recv()
	model := r.model()
	svc := "r.client." + cfg.ClientField
	pkg := cfg.BindingsPkg
	grant := r.Create.Method        // e.g. "GrantCsTeamsMeetingPolicy"
	grantP := r.Create.Params       // e.g. "GrantCsTeamsMeetingPolicyParams"
	userField := r.grantUserField() // grant params field for the user, e.g. "Identity"
	ptype := r.AssignmentPolicyType
	readFn := r.assignmentUserRead()
	// Human-facing cmdlet name for diagnostics, derived from the grant method
	// (e.g. "GrantCsTeamsMeetingPolicy" -> "Grant-CsTeamsMeetingPolicy").
	grantCmdlet := grant
	if len(grant) >= 5 && grant[:5] == "Grant" {
		grantCmdlet = "Grant-" + grant[5:]
	}
	const readErr = "reading the effective policy assignment failed"

	fmt.Fprintf(&b, "// Code generated by gen-tf; DO NOT EDIT.\n\n")
	fmt.Fprintf(&b, "package %s\n\n", cfg.Package)

	// Imports — exactly what the body references.
	fmt.Fprintf(&b, "import (\n")
	fmt.Fprintf(&b, "\t%q\n", "context")
	fmt.Fprintf(&b, "\t%q\n\n", "time")
	fmt.Fprintf(&b, "\t%q\n", "github.com/hashicorp/terraform-plugin-framework/diag")
	...
	fmt.Fprintf(&b, "// %s manages the per-user assignment of a %s instance (%s).\n",
		r.ctor(), ptype, grantCmdlet)
	fmt.Fprintf(&b, "func %s() resource.Resource { return &%s{} }\n\n", r.ctor(), recv)

	// Model: id + user + policy_name.
	fmt.Fprintf(&b, "type %s struct {\n", model)
	fmt.Fprintf(&b, "\tID types.String `tfsdk:\"id\"`\n")
	fmt.Fprintf(&b, "\tUser types.String `tfsdk:\"user\"`\n")
	fmt.Fprintf(&b, "\tPolicyName types.String `tfsdk:\"policy_name\"`\n")
	fmt.Fprintf(&b, "}\n\n")
	...
	fmt.Fprintf(&b, "\t\tAttributes: map[string]schema.Attribute{\n")
	fmt.Fprintf(&b, "\t\t\t%q: schema.StringAttribute{Computed: true, Description: %q, PlanModifiers: []planmodifier.String{stringplanmodifier.UseStateForUnknown()}},\n",
		"id", "Assignment identifier (the user's object id or user principal name).")
	fmt.Fprintf(&b, "\t\t\t%q: schema.StringAttribute{Required: true, Description: %q, PlanModifiers: []planmodifier.String{stringplanmodifier.RequiresReplace()}},\n",
		"user", "User the policy is assigned to (object id or user principal name). Changing it forces replacement.")
	fmt.Fprintf(&b, "\t\t\t%q: schema.StringAttribute{Required: true, Description: %q},\n",
		"policy_name", "Name of the "+ptype+" instance to assign to the user.")
	fmt.Fprintf(&b, "\t\t},\n\t}\n}\n\n")
	...
	fmt.Fprintf(&b, "\tresp.Diagnostics.Append(req.Plan.Get(ctx, &plan)...)\n")
	fmt.Fprintf(&b, "\tif resp.Diagnostics.HasError() {\n\t\treturn\n\t}\n")
	fmt.Fprintf(&b, "\tuser := plan.User.ValueString()\n")
	fmt.Fprintf(&b, "\tname := plan.PolicyName.ValueString()\n")
	fmt.Fprintf(&b, "\tif _, err := %s.%s(ctx, %s.%s{%s: user, PolicyName: name}); err != nil {\n", svc, grant, pkg, grantP, userField)
	fmt.Fprintf(&b, "\t\tresp.Diagnostics.AddError(%q, err.Error())\n\t\treturn\n\t}\n", grantCmdlet+" failed")
	fmt.Fprintf(&b, "\tif !r.waitAssigned(ctx, user, name, &resp.Diagnostics) {\n\t\treturn\n\t}\n")
	fmt.Fprintf(&b, "\tplan.ID = types.StringValue(user)\n")
	fmt.Fprintf(&b, "\tresp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)\n}\n\n")
	...
	fmt.Fprintf(&b, "\tresp.Diagnostics.Append(req.State.Get(ctx, &state)...)\n")
	fmt.Fprintf(&b, "\tif resp.Diagnostics.HasError() {\n\t\treturn\n\t}\n")
	fmt.Fprintf(&b, "\tuser := state.User.ValueString()\n")
	fmt.Fprintf(&b, "\tname, err := %s.%s(ctx, user, %q)\n", svc, readFn, ptype)
	fmt.Fprintf(&b, "\tif err != nil {\n\t\tresp.Diagnostics.AddError(%q, err.Error())\n\t\treturn\n\t}\n", readErr)
	// Empty ⇒ no explicit assignment (user reverted to global, or was deleted): drift.
	fmt.Fprintf(&b, "\tif name == \"\" {\n\t\tresp.State.RemoveResource(ctx)\n\t\treturn\n\t}\n")
	fmt.Fprintf(&b, "\tstate.ID = types.StringValue(user)\n")
	fmt.Fprintf(&b, "\tstate.PolicyName = types.StringValue(name)\n")
	fmt.Fprintf(&b, "\tresp.Diagnostics.Append(resp.State.Set(ctx, &state)...)\n}\n\n")
	...
	// Delete: unassign by granting an empty PolicyName.
	fmt.Fprintf(&b, "func (r *%s) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {\n", recv)
	fmt.Fprintf(&b, "\tvar state %s\n", model)
	fmt.Fprintf(&b, "\tresp.Diagnostics.Append(req.State.Get(ctx, &state)...)\n")
	fmt.Fprintf(&b, "\tif resp.Diagnostics.HasError() {\n\t\treturn\n\t}\n")
	fmt.Fprintf(&b, "\tif _, err := %s.%s(ctx, %s.%s{%s: state.User.ValueString(), PolicyName: \"\"}); err != nil {\n", svc, grant, pkg, grantP, userField)
	fmt.Fprintf(&b, "\t\tif !isNotFound(err) {\n\t\t\tresp.Diagnostics.AddError(%q, err.Error())\n\t\t}\n\t}\n}\n\n", grantCmdlet+" (unassign) failed")

	// ImportState: the import id is the user; id and user both take it.
	...
	// waitAssigned polls the effective assignment until it reflects the granted
	// instance. Grants are asynchronous (~30-45s to surface), so the retry budget
	// is larger than the shared default; a timeout leaves the just-written plan
	// value authoritative (the next Read reconciles), and only a fatal read error
	// fails the apply.
	fmt.Fprintf(&b, "func (r *%s) waitAssigned(ctx context.Context, user, want string, diags *diag.Diagnostics) bool {\n", recv)
	fmt.Fprintf(&b, "\t_, _, err := resourcex.LoadUntil(ctx, consistency.Config{Attempts: 20, Delay: 4 * time.Second}, func(ctx context.Context) (string, bool, error) {\n")
	fmt.Fprintf(&b, "\t\tgot, gerr := %s.%s(ctx, user, %q)\n", svc, readFn, ptype)
	fmt.Fprintf(&b, "\t\tif gerr != nil {\n\t\t\treturn \"\", false, gerr\n\t\t}\n")
	fmt.Fprintf(&b, "\t\treturn got, got != \"\", nil\n")
	fmt.Fprintf(&b, "\t}, func(got string) bool { return got == want })\n")
